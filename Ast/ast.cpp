    ...
    : Expression(NodeType::LOG, ExprType::BOOL, t), expr1(e1), expr2(e2)
{
    ...
    {
        stringstream ss;
        ss << "\'" << t->lexeme << "\' usado con operandos no booleanos ("
...
{
    // Si es negación lógica (!) y el operando es int para convertir a bool
    if (t->lexeme == "!" && expr->type == ExprType::INT)
    {
        expr = new IntToBool(expr);
    }
    // verificación de tipos
    if (expr->type != ExprType::BOOL && t->lexeme == "!")
    {
        stringstream ss;
        ss << "\'" << t->lexeme << "\' usado con operando no booleano ("
           << expr->Name() << ":" << expr->Type() << ")";
        throw SyntaxError{scanner->Lineno(), ss.str()};
    }
}
...
{
    // COERCIÓN: permitir asignar int a bool
